...
function comparePasswords() {
  if (passwordInput.value !== confirmInput.value) {
    ...
    confirmInput.className =
      passwordInput.validity.customError && confirmInput.value.length !== 0
        ? "error-line"
        : "";
  } else {
...
function showErrors() {
  ...
  Array.from(inputs).forEach((input) => {
    ...
    // check is input value too shorst
      ...
      showInputMessage(
        input,
        `Input field requires at least ${input.minLength} characters!`
      );
    }
...
formElement.addEventListener("submit", (e) => {
  ...
  if (!formMsgElement.classList.contains("hidden"))
    formMsgElement.classList.add("hidden");
